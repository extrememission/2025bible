...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Check initial state
    checkOfflineStatus().then(isReady => {
        if (isReady) {
            offlineToggle.checked = true;
            offlineToggle.disabled = true;
            document.querySelector('.slider').style.backgroundColor = 'rgba(0, 255, 0, 0.3)';
        }
    });
    ...
    offlineToggle.addEventListener('change', async (e) => {
        if (e.target.checked) {
            try {
                document.querySelector('.slider').style.backgroundColor = 'rgba(255, 0, 0, 0.3)';
                const cache = await caches.open('bible-v2');
